...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	runSpentCalories := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return runSpentCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	meanSpeedSquared := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2) // средняя скорость в квадрате
	walkSpentCalories := ((walkingCaloriesWeightMultiplier*weight + (meanSpeedSquared/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkSpentCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	swimmSpentCalories := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimmSpentCalories
}
